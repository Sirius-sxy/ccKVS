...
{
	printf("CLIENT MODE: Starting ccKVS client\n");
	...
	if (WRITE_RATIO > 0 && DISABLE_CACHE == 0)
		assert(BCAST_TO_CACHE_BATCH <= CACHE_BATCH_SIZE);

	assert(SC_CREDITS % SC_CREDIT_DIVIDER == 0);
	assert(CREDITS_FOR_EACH_CLIENT % LIN_CREDIT_DIVIDER == 0);
	assert(MAX_COH_MESSAGES < K_64);
	assert(SESSIONS_PER_CLIENT * CLIENTS_PER_MACHINE <= 255);
	assert(WS_PER_WORKER < 256);
	assert(LOCAL_WINDOW % LOCAL_REGIONS == 0);
	assert(LOCAL_REGIONS <= 64);
	...
	if (CLIENT_ENABLE_INLINING == 1)
		assert((MAX_COALESCE_PER_MACH * HERD_GET_REQ_SIZE) + 1 <= MAXIMUM_INLINE_SIZE);

	int i, c;
	is_master = -1; is_client = 1; // CLIENT: is_client = 1
	int num_threads = -1;
	...
	latency_count.hot_writes  = (uint32_t*) malloc(sizeof(uint32_t) * (LATENCY_BUCKETS + 1));
	latency_count.hot_reads   = (uint32_t*) malloc(sizeof(uint32_t) * (LATENCY_BUCKETS + 1));
	latency_count.local_reqs  = (uint32_t*) malloc(sizeof(uint32_t) * (LATENCY_BUCKETS + 1));
	latency_count.remote_reqs = (uint32_t*) malloc(sizeof(uint32_t) * (LATENCY_BUCKETS + 1));
	...
	for(i = 0; i < CLIENTS_PER_MACHINE; i++) {
		...
		pthread_create(&thread_arr[i], &attr, run_client, &param_arr[i]);
		occupied_cores[c_core] = 1;
	}

	if (ENABLE_SS_DEBUGGING == 1) {
		if (CREDITS_IN_MESSAGE != CREDITS_FOR_EACH_CLIENT)
			red_printf("CREDITS IN MESSAGE is bigger than 1: %d, that could cause a deadlock.. \n",
					   CREDITS_IN_MESSAGE);
		if (MAX_CREDIT_WRS >= MIN_SS_BATCH)
			printf("MAX_CREDIT_WRS is %d, CREDIT_SS_BATCH is %d \n", MAX_CREDIT_WRS, CREDIT_SS_BATCH);
		if (SC_MAX_CREDIT_WRS >= MIN_SS_BATCH)
			printf("SC_MAX_CREDIT_WRS is %d, SC_CREDIT_SS_BATCH is %d \n", SC_MAX_CREDIT_WRS, SC_CREDIT_SS_BATCH);
		if (WORKER_MAX_BATCH >= MIN_SS_BATCH)
			printf("WORKER_MAX_BATCH is %d, WORKER_SS_BATCH is %d \n", WORKER_MAX_BATCH, WORKER_SS_BATCH);
		if (WINDOW_SIZE >= MIN_SS_BATCH)
			printf("WINDOW_SIZE is %d, CLIENT_SS_BATCH is %d \n", WINDOW_SIZE, CLIENT_SS_BATCH);
		if (MESSAGES_IN_BCAST_BATCH >= MIN_SS_BATCH)
			printf("MESSAGES_IN_BCAST_BATCH is %d, BROADCAST_SS_BATCH is %d \n",
				   MESSAGES_IN_BCAST_BATCH, BROADCAST_SS_BATCH);
		if (BCAST_TO_CACHE_BATCH >= MIN_SS_BATCH)
			printf("BCAST_TO_CACHE_BATCH is %d, ACK_SS_BATCH is %d \n", BCAST_TO_CACHE_BATCH, ACK_SS_BATCH);
	}
