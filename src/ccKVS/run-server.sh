#!/usr/bin/env bash
# Script to run ccKVS server (workers only) in separated mode

### Start of initialization ###
is_RoCE=0
protocol="sc" # choose "sc" (Sequential Consistency) or "lin" (Linearizability)
executable="ccKVS-server-${protocol}"
...
# A function to echo in green color
function green() {
	es=`tput setaf 2`
	ee=`tput sgr0`
	echo "${es}$1${ee}"
...
blue "SERVER MODE: Starting ccKVS server"
...
sudo killall ccKVS-server-sc
sudo killall ccKVS-server-lin
memcached -l 0.0.0.0 1>/dev/null 2>/dev/null &
sleep 1

green "Running server (worker threads only)"
